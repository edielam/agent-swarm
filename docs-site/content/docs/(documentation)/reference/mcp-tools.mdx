...
- **Core tools** (16) — Always available in context: task lifecycle, basic communication, memory recall, debug, and swarm awareness
- **Deferred tools** (73) — Discovered on demand via Claude Code's Tool Search when the agent needs them (scheduling, epics, config, skills, workflows, MCP servers, file operations, etc.)
